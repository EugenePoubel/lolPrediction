...
function Prediction() {
    ...
    const [championsNames, setChampionNames] = useState([]);
    const { pseudo } = useParams();
    const api_key = "RGAPI-2b2d0a4a-7f33-4d84-8bdc-9f89994df4ce"
    useEffect(() => {
            ...
            .then(id => {
                ...
                getChampionData()
                    ...
                    .then(gameInfo => {
                        ...
                        if (gameInfo) {
                            const names = gameInfo.participants.map(participant => {
                                return getChampionName(participant.championId, championData);
                            });
                            setChampionNames(names);
                        } else {
                            console.log(pseudo + " is not currently in a game.");
                        }
                    })
    ...
    function getChampionData() {
        const url = 'https://ddragon.leagueoflegends.com/cdn/11.22.1/data/en_US/champion.json';
        return fetch(url)
            .then(response => response.json())
            .then(data => data.data)
            .catch(error => {
